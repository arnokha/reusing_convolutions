...
options = {"game":"YarsRevenge-v0",            ## Game to train policy on
           ...
           "n_episodes":2,              ## Number of episodes to train for
...
if len(sys.argv) != 4:
    ...
    print(options["game"], options["n_channels_out_1"], options["n_channels_out_2"])
...
n_dim_x_d2 = observation[::2,:,:].shape[0]
n_dim_y_d2 = observation[:,::2,:].shape[1]
...
text_file = open(options["game"].rsplit("-")[0] + "/test_reuse_conv_cpu_{}_{}_out.txt".format(options["n_channels_out_1"],
